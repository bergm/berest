...
(defn- plot-content [user-id id until]
  #_(let? [db (bd/current-db user-id)
         ...
         weather (climate/sorted-weather-data-map db "zalf/zalf" 1993)
  ...
  (let [wd (rest (csv/parse-csv weather-data
           ...
           create-map (if (-> wd first count (= ,,, 3))
                       ...
                       (fn [doy precip globrad tavg]
                         {:doy (Integer/parseInt doy)
                          :precipitation (Float/parseFloat precip)
                          :evaporation (c-algo/potential-evaporation-turc-wendling
...
(defn calc-test-plot
  ...
  #_(let? [db (bd/current-db user-id)
         ...
         weather (climate/sorted-weather-data-map db "zalf/zalf" weather-year*)
